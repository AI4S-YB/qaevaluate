...
LOCAL_IMPL_PATH = ROOT / "local_scripts" / "reset_domain_sample_data_impl.py"


def _missing_impl_message() -> str:
    return (
        f"missing local AI4S implementation: {LOCAL_IMPL_PATH}. "
        "This repo keeps AI4S / 8-domain QA generation logic in a local ignored directory."
    )
...
def _load_local_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "local_reset_domain_sample_data_impl",
        LOCAL_IMPL_PATH,
    )
